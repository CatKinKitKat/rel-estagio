 ...
 * Used to access the Webshop table with JOOQ type safe operations via the R2DBC
 * driver in a reactive fashion.
 *
 * @author G. Amaro
 */
@Log
...
public class WebshopRepository implements com.optiply.infrastructure.data.repositories.interfaces.IWebshopRepository {
  ...
  /**
   * The DSL context, used to generate SQL queries or get its dialect and other
   * properties.
   */
  private final DSLContext dslContext;
  /**
   * The R2DBC driver, used to execute SQL queries in a reactive fashion, needs
   * DSLContext properties.
   */
    ...
    return Mono.from(operations.withTransaction(
        ...
        status -> Mono
            .from(DSL
                ...
                .columns(Tables.WEBSHOP.HANDLE, Tables.WEBSHOP.URL, Tables.WEBSHOP.A, Tables.WEBSHOP.B,
                    Tables.WEBSHOP.C, Tables.WEBSHOP.INTEREST_RATE, Tables.WEBSHOP.CURRENCY, Tables.WEBSHOP.RUN_JOBS,
                    Tables.WEBSHOP.MULTI_SUPPLY)
                .values(handle, url, serviceLevelA, serviceLevelB, serviceLevelC, interestRate, currency, runJobs,
                    multiSupplier))
  ...
  /**
   * Find various webshops by a given set of conditions calculated by the url
   * query parameters.
   *
  ...
  public Flux<String> findVarious(Condition condition, SortField<?> sort) {
    ...
    return Flux.from(operations.withTransaction(TransactionDefinition.READ_ONLY, status -> DSL
        ...
        .from(Tables.WEBSHOP)
        .where(condition)
        .orderBy(sort))).map(result -> result.into(Webshop.class)).map(Webshop::getHandle);
  }
