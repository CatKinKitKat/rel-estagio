...
public class RepositoryService {
		...
		return webshopRepository
				...
				.flatMap(webshopModel -> webshopemailsRepository.findEmails(handle)
						.flatMap(webshopemails -> {
							webshopModel.setEmails(webshopemails);
							return Mono.just(webshopModel);
						}).flatMap(webshopModelWithEmails -> Mono.just(HttpResponse.ok(webshopModelWithEmails))))
				.switchIfEmpty(Mono.just(HttpResponse.notFound()))
				.onErrorReturn(HttpResponse.serverError());
	}
	...
	public Mono<MutableHttpResponse<WebshopSettingsModel>> getWebshopSettings(String handle) {
		...
		return webshopRepository.find(handle)
				.flatMap(webshop -> Mono.just(new WebshopSettingsModel(webshop)))
	...
	public Mono<MutableHttpResponse<String>> createWebshop(WebshopFullModel webshopModel) {
		...
		return webshopRepository.create(
				...
				webshopModel.getMultiSupplier()).flatMap(webshopResponse -> {
					if (webshopResponse) {
				...
				.publishOn(Schedulers.boundedElastic()).flatMap(webshops -> {
					...
					for (WebshopFullModel webshopModel : webshops) {
						webshopRepository.create(
								...
								webshopModel.getServiceLevelC(), webshopModel.getInterestRate(),
								webshopModel.getCurrency(), webshopModel.getRunJobs(),
								webshopModel.getMultiSupplier()).subscribeOn(Schedulers.boundedElastic())
	 ...
	 * @return the mono
	 */
	public Mono<MutableHttpResponse<String>> updateWebshop(String handle, WebshopFullModel webshopModel) {
		...
		return webshopRepository.updateWebshop(handle, webshopModel.getHandle(),
				...
				webshopModel.getRunJobs(), webshopModel.getMultiSupplier()).flatMap(response -> {
					if (response) {
						return webshopemailsRepository.deleteAll(webshopModel.getHandle())
								.flatMap(deleteResponse -> webshopemailsRepository.createVarious(webshopModel.getHandle(),
	 ...
	 * @return the mono
	 */
	public Mono<MutableHttpResponse<String>> updateWebshopServiceLevels(String handle,
			ServiceLevelsModel serviceLevelsModel) {
